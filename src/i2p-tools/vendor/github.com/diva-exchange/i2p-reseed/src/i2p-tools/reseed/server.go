...
func NewServer(prefix string, trustProxy bool) *Server {
	...
	mux.Handle(prefix+"/i2pseeds.su3", middlewareChain.Append(disableKeepAliveMiddleware, loggingMiddleware, verifyMiddleware).Then(http.HandlerFunc(server.reseedHandler)))
	server.Handler = mux
